...
#         ./mcperf -T 2 -A
#     "
    gcloud compute ssh --ssh-key-file ~/.ssh/cloud-computing ubuntu@$NAME_AGENT_A --zone europe-west1-b --command "cd memcache-perf-dynamic/ && ./mcperf -T 2 -A"
...
#         ./mcperf -T 4 -A
#     "
    gcloud compute ssh --ssh-key-file ~/.ssh/cloud-computing ubuntu@$NAME_AGENT_B --zone europe-west1-b --command "cd memcache-perf-dynamic/ && ./mcperf -T 4 -A"
...
#     "
    gcloud compute ssh --ssh-key-file ~/.ssh/cloud-computing ubuntu@$CLIENT_MEASURE_NAME --zone europe-west1-b --command "cd memcache-perf-dynamic/ && ./mcperf -s ${MEMCACHED_IP} --loadonly && ./mcperf -s ${MEMCACHED_IP} -a ${INTERNAL_AGENT_A_IP} -a ${INTERNAL_AGENT_B_IP} --noload -T 6 -C 4 -D 4 -Q 1000 -c 4 -t 10 --scan 30000:30500:5"
fi
